...
Component skeleton = Component("skeleton/torso.ply");
Vector **right_leg_targets = new Vector *[4];
Vector **left_leg_targets = new Vector *[4];
Vector **right_arm_targets = new Vector *[4];
Vector **left_arm_targets = new Vector *[4];
int right_leg_target_index = 1;
int left_leg_target_index = 3;
int right_arm_target_index = 1;
int left_arm_target_index = 3;
...
void draw_limb(Component *upper, float upper_angle, float lower_angle, float extremity_angle) {
    Component *lower_leg = upper->children[0];
    Component *foot = lower_leg->children[0];

    // Hip rotation
    glPushMatrix();
    rotate_matrix(upper, upper_angle, 1, 0, 0);
    upper->parent_joint_angle = new Vector(upper_angle, 0, 0);
    draw_component(upper);

    // Knee rotation
    glPushMatrix();
    rotate_matrix(lower_leg, lower_angle, 1, 0, 0);
    lower_leg->parent_joint_angle = new Vector(lower_angle, 0, 0);
    float x = upper->parent_joint_vertex->values[0];
    float y = upper->length * sin(rad(upper_angle)) + upper->parent_joint_vertex->values[1];
    float z = -upper->length * cos(rad(upper_angle)) + upper->parent_joint_vertex->values[2];
    lower_leg->parent_joint_vertex = new Vector(x, y, z);
    draw_component(lower_leg);

    // Ankle rotation
    glPushMatrix();
    rotate_matrix(foot, extremity_angle, 1, 0, 0);
    foot->parent_joint_angle = new Vector(extremity_angle, 0, 0);
    x = lower_leg->parent_joint_vertex->values[0];
    y = lower_leg->length * sin(rad(180 - (upper_angle + lower_angle))) + lower_leg->parent_joint_vertex->values[1];
    z = lower_leg->length * cos(rad(180 - (upper_angle + lower_angle))) + lower_leg->parent_joint_vertex->values[2];
...
int target(Component *upper, Vector **target_list, int target_index) {
    Component *lower = upper->children[0];
    Component *extremity = lower->children[0];
    ...
    float y0 = upper->parent_joint_vertex->values[1];
    float z0 = upper->parent_joint_vertex->values[2];
    float y1 = lower->parent_joint_vertex->values[1];
    float z1 = lower->parent_joint_vertex->values[2];
    float y2 = extremity->parent_joint_vertex->values[1];
    float z2 = extremity->parent_joint_vertex->values[2];
    float theta0 = upper->parent_joint_angle->values[0];
    float theta1 = lower->parent_joint_angle->values[0];
    ...
    if (d_upper < d_lower) {
        ...
        printf("Target: %d\n", target_index);
        printf("Upper: %f (at %f)\n", theta0_prime, theta0);
        draw_limb(upper, theta0_prime, theta1, 0);
    } else {
        // Check for and set new target
        ...
        printf("Target: %d\n", target_index);
        printf("Lower: %f (at %f)\n", theta1_prime, theta1);
        draw_limb(upper, theta0, theta1_prime, 0);
    }
...
void display() {
    ...
    Component *left_leg = skeleton.children[0];
    Component *right_leg = skeleton.children[1];
    Component *left_arm = skeleton.children[2];
    Component *right_arm = skeleton.children[3];
    ...
    right_arm_target_index = target(right_arm, right_arm_targets, right_arm_target_index);
    left_arm_target_index = target(left_arm, left_arm_targets, left_arm_target_index);

    draw_component(skeleton.children[4]);
...
int main(int argc, char **argv) {
    ...
    Component *right_arm_upper = new Component("skeleton/right_arm_upper.ply", -5.8287, 1.5549, 11.184);
    skeleton.add_child(right_arm_upper);
    Component *right_arm_lower = new Component("skeleton/right_arm_lower.ply", -9.76, 3.111, -0.3137);
    right_arm_upper->length = distance(right_arm_upper->parent_joint_vertex, right_arm_lower->parent_joint_vertex);
    right_arm_upper->add_child(right_arm_lower);
    Component *right_hand = new Component("skeleton/right_hand.ply", -9.7725, 0.9786, -9.1379);
    right_arm_lower->length = distance(right_arm_lower->parent_joint_vertex, right_hand->parent_joint_vertex);
    right_arm_lower->add_child(right_hand);

    Component *left_arm_upper = new Component("skeleton/left_arm_upper.ply", 5.8287, 1.5549, 11.184);
    skeleton.add_child(left_arm_upper);
    Component *left_arm_lower = new Component("skeleton/left_arm_lower.ply", 9.76, 3.111, -0.3137);
    left_arm_upper->length = distance(left_arm_upper->parent_joint_vertex, left_arm_lower->parent_joint_vertex);
    left_arm_upper->add_child(left_arm_lower);
    Component *left_hand = new Component("skeleton/left_hand.ply", 9.7725, 0.9786, -9.1379);
    left_arm_lower->length = distance(left_arm_lower->parent_joint_vertex, left_hand->parent_joint_vertex);
    left_arm_lower->add_child(left_hand);

    Component *skull = new Component("skeleton/head.ply");
    ...
    right_arm_targets[0] = new Vector(-9.7725, 0.9786, -9.1379);
    right_arm_targets[1] = new Vector(-9.7725, -10, 0);
    right_arm_targets[2] = new Vector(-9.7725, 0.9786, -9.1379);
    right_arm_targets[3] = new Vector(-9.7725, 7, 4);

    left_arm_targets[0] = new Vector(9.7725, 0.9786, -9.1379);
    left_arm_targets[1] = new Vector(9.7725, 0.9786, -9.1379);
    left_arm_targets[2] = new Vector(9.7725, 0.9786, -9.1379);
    left_arm_targets[3] = new Vector(9.7725, 0.9786, -9.1379);
    ...
    delete[] right_arm_targets;
    delete[] left_arm_targets;
    return 0;
}
